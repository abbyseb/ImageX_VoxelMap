...
class net2Dto3D(nn.Module):
    ...
    def __init__(self, im_size):
        ...
        enc_nf = []
        for nb in range(2, int(np.log2(im_size)) + 2):  # Calculates how many times before the image becomes 1
            enc_nf.append(2 ** nb) # as the image gets smaller the image becomes bigger

        dec_nf = enc_nf[::-1]  # Reverse of Encoder List
        dec_nf.append(3) #Hard-codes the final output to be 3 dimensions (X, Y, Z).

        self.enc_nf, self.dec_nf = enc_nf, dec_nf

        # configure encoder (down-sampling path)
        prev_nf = 2 # Start with 2 channels as we input 2 images
        self.downarm = nn.ModuleList()
        for nf in self.enc_nf:
            self.downarm.append(DownBlock(prev_nf, nf)) # loop of new down blocks  to gather feature from image
            prev_nf = nf
